...
void Plant_From_Input_And_Feedback (void)
{
    ...
    // pid_control.ki = 4;    //entre 2 4 y 6
    // float ki_gain = gain_five_perc * 6.28 * 128./ fsampling;

    // calc ki gain with kp 
    // float ki_gain = gain_five_perc * 6.28 * 1.5 * 128./ fsampling;
    ...
    // float ki_gain = gain_five_perc * 6.28 * 20. * 128.;    
    pid_control.ki = (short) ki_gain;    //ki = kp * 2 * np.pi * 20 / fs    
    pid_control.kp = (short) (gain_five_perc * 128);
    // pid_control.kp = 40;    
    pid_control.kd = 0;
    ...
    // printf(" pid values ki_dig: %f kp_dig: %f kd_dig: %f\n", ki_dig / 128., kp_dig / 128., kd_dig / 128.);
    ...
    printf(" zero from ki_eff: %f to kp: %fHz\n",
           (float) pid_control.ki * fsampling,
           (float) pid_control.ki * fsampling / (6.28 * (float) pid_control.kp));
    
    PID_Clear (&pid_control);

    float freq_selected = 20.0;
